  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {arcs.map((a, i) => (
          <Motion
            defaultStyle={{ c: 0, r: 0, load: 0, test: 0 }}
            style={{
              load: spring(1, { stiffness: 200, damping: 5 }),
              c: spring(hovered === i ? 1 : 0, { stiffness: 1000, damping: 26 }), // 500 1
              test: spring(hovered === i ? 1 : 0, { stiffness: 100, damping: 5 }),
          ...
          >
            {({ c, r, load, test }) => {
              ...
              const inner = smallInnerRadius - (r * 30) - (test * 10)
              const newR = smallRadius + (radius - smallRadius) * test
              return (
                <g key={`arc${i}`} transform={`translate(${xDir * r}, ${yDir * r})`}>
                  <path
                    d={arcGen(newR, inner)(actualA)}
                    fill={colors(a, i, c)}
